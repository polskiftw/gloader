...
internal static class RadioUi
{
    ...
    private static string _view = "radiomonster";
    ...
    internal static void TryInstall(Harmony harmony)
    {
        try
        {
            ...
            if (close != null)
                harmony.Patch(close, postfix: new HarmonyMethod(AccessTools.Method(typeof(RadioUi), nameof(IngameOptionsClosePostfix))));
    ...
    private static void IngameOptionsPostfix(object[] __args)
    {
        ...
        try
        {
            ...
            {
                var selected = GeneralRadio.SelectedStation;
                if (selected != null && (selected.Provider == "radiomonster" || selected.Provider == "rainwave"))
                    _view = selected.Provider;
                _page = 0;
                IsOpen = true;
            });
        }
    ...
    private static void DrawBrowser(object spriteBatch)
    {
        ...
        var left = Math.Max(24, screenWidth / 2 - 500);
        var top = Math.Max(20, screenHeight / 2 - 330);
        var width = Math.Min(1000, screenWidth - left * 2);
        ...
        DrawText(spriteBatch, "RadioMonster.fm + Rainwave  ·  two sources, nothing else", left + 112, top + 18, 185, 195, 215, 255, 0.85f);
        DrawButton(spriteBatch, left + width - 90, top + 12, 70, 28, "Back", false, () => IsOpen = false);

        var tabsY = top + 58;
        DrawButton(spriteBatch, left + 20, tabsY, 180, 30, "RadioMonster.fm", _view == "radiomonster", () => { _view = "radiomonster"; _page = 0; });
        DrawButton(spriteBatch, left + 208, tabsY, 150, 30, "Rainwave", _view == "rainwave", () => { _view = "rainwave"; _page = 0; });
        DrawButton(spriteBatch, left + 366, tabsY, 140, 30, "Favorites", _view == "favorites", () => { _view = "favorites"; _page = 0; });
        DrawButton(spriteBatch, left + width - 116, tabsY, 96, 30,
            GeneralRadio.State.SongNotifications ? "Popup ✓" : "Popup ✕",
            GeneralRadio.State.SongNotifications,
            GeneralRadio.ToggleNotifications);

        var contentX = left + 20;
        var contentY = top + 106;
        var contentWidth = width - 40;
        var nowY = top + height - 80;
        DrawStationRows(spriteBatch, contentX, contentY, contentWidth, nowY - contentY - 8);
        DrawNowPlayingStrip(spriteBatch, contentX, nowY, contentWidth, 62);
        DrawVanillaCursor();
    }
    ...
    private static void DrawStationRows(object spriteBatch, int x, int y, int width, int availableHeight)
    {
        ...
        if (_view == "favorites")
            filtered = all.Where(station => favorites.Contains(station.Id)).ToList();
        else
            filtered = all.Where(station => string.Equals(station.Provider, _view, StringComparison.OrdinalIgnoreCase)).ToList();

        var rowsPerPage = Math.Max(4, Math.Min(10, availableHeight / 38));
        ...
        var heading = _view == "favorites" ? "Favorite stations" : (_view == "rainwave" ? "Rainwave · 6 stations" : "RadioMonster.fm · 10 stations");
        DrawText(spriteBatch, heading, x, y - 18, 170, 185, 205, 255, 0.74f);

        if (rows.Count == 0)
        {
            DrawText(spriteBatch, "No favorites yet.", x + 10, y + 14, 185, 195, 215, 255, 0.82f);
            return;
        }
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            DrawText(spriteBatch, station.Description + "  ·  " + station.ProviderDisplay, x + 10, rowY + 19, 150, 175, 205, 255, 0.66f);
    ...
    private static void DrawNowPlayingStrip(object spriteBatch, int x, int y, int width, int height)
    {
        ...
        var stationName = station == null ? "No station" : station.ProviderDisplay + " · " + station.Name;
        var trackText = track == null
            ? (GeneralRadio.Health == RadioHealth.MetadataUnavailable ? "Track metadata unavailable" : "Waiting for track metadata…")
            : track.Display;
        ...
        DrawText(spriteBatch,
            GeneralRadio.Health + "  ·  " + GeneralRadio.StatusDetail + (string.IsNullOrWhiteSpace(GeneralRadio.ActiveStreamLabel) ? "" : "  ·  " + GeneralRadio.ActiveStreamLabel),
            x + 12, y + 44, 145, 165, 190, 255, 0.64f);
    ...
    {
        try { AccessTools.Field(GeneralRadio.MainType, "mouseLeftRelease")?.SetValue(null, false); }
        catch { }
    }
    ...
    {
        try { return AccessTools.Field(GeneralRadio.MainType, field)?.GetValue(null); }
        catch { return null; }
    }
