...
public static class AnimatedImageProcessor
{
    ...
    public static ImageStorageResult PackFrames(IReadOnlyList<byte[]> framePngs, IReadOnlyList<int> durationsMs, int repetitionCount)
        => PackFramesCancellable(framePngs, durationsMs, repetitionCount, CancellationToken.None);

    public static ImageStorageResult PackFramesCancellable(IReadOnlyList<byte[]> framePngs, IReadOnlyList<int> durationsMs, int repetitionCount, CancellationToken cancellationToken)
    {
    ...
    public static List<byte[]> ExtractFrames(ReadOnlySpan<byte> atlasPng, GelConfig config)
        => ExtractFramesCancellable(atlasPng, config, CancellationToken.None);

    public static List<byte[]> ExtractFramesCancellable(ReadOnlySpan<byte> atlasPng, GelConfig config, CancellationToken cancellationToken)
    {
        if (!IsAnimated(config)) return [atlasPng.ToArray()];
        var atlas = RawRgbaCodec.Decode(atlasPng);
        ValidateAtlas(config, atlas.Width, atlas.Height);
    ...
    public static ImageStorageResult TransformAnimated(ReadOnlySpan<byte> atlasPng, GelConfig config, Func<byte[], byte[]> transform)
        => TransformAnimatedCancellable(atlasPng, config, transform, CancellationToken.None);

    public static ImageStorageResult TransformAnimatedCancellable(ReadOnlySpan<byte> atlasPng, GelConfig config, Func<byte[], byte[]> transform, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(transform);
        if (!IsAnimated(config)) throw new GelFormatException("The requested operation requires an animated GEL asset.");
        var animation = config.Animation!;
        var frames = ExtractFramesCancellable(atlasPng, config, cancellationToken);
        ...
        return PackFramesCancellable(transformed, animation.Frames.Select(frame => frame.DurationMs).ToArray(), animation.RepetitionCount, cancellationToken);
    }
    ...
    public static ImageStorageResult TransformFrame(ReadOnlySpan<byte> atlasPng, GelConfig config, int frameIndex, Func<byte[], byte[]> transform)
        => TransformFrameCancellable(atlasPng, config, frameIndex, transform, CancellationToken.None);

    public static ImageStorageResult TransformFrameCancellable(ReadOnlySpan<byte> atlasPng, GelConfig config, int frameIndex, Func<byte[], byte[]> transform, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(transform);
        if (!IsAnimated(config)) throw new GelFormatException("The requested operation requires an animated GEL asset.");
        var animation = config.Animation!;
    ...
    public static PixelRect? FindUnionTrimBounds(ReadOnlySpan<byte> atlasPng, GelConfig config, double alphaThreshold)
        => FindUnionTrimBoundsCancellable(atlasPng, config, alphaThreshold, CancellationToken.None);

    public static PixelRect? FindUnionTrimBoundsCancellable(ReadOnlySpan<byte> atlasPng, GelConfig config, double alphaThreshold, CancellationToken cancellationToken)
    {
        if (!IsAnimated(config)) return RawRgbaTransforms.FindTrimBoundsCancellable(atlasPng, alphaThreshold, cancellationToken);
        var atlas = RawRgbaCodec.Decode(atlasPng);
        ValidateAtlas(config, atlas.Width, atlas.Height);
    ...
    public static byte[] BuildUnionAlphaPng(ReadOnlySpan<byte> atlasPng, GelConfig config)
        => BuildUnionAlphaPngCancellable(atlasPng, config, CancellationToken.None);

    public static byte[] BuildUnionAlphaPngCancellable(ReadOnlySpan<byte> atlasPng, GelConfig config, CancellationToken cancellationToken)
    {
        if (!IsAnimated(config)) return atlasPng.ToArray();
        var atlas = RawRgbaCodec.Decode(atlasPng);
        ValidateAtlas(config, atlas.Width, atlas.Height);
